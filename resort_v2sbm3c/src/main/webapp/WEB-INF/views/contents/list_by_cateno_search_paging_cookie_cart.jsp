...
<DIV class='title_line'>
  <A href="./list_by_cateno_search_paging.do?cateno=${cateVO.cateno}" class='title_link'> 한국 프리미어리그 선수</A>
</DIV>
      ...
      <c:forEach var="contentsVO" items="${list }">
        ...
        <tr style="height: 132px;"> 
              ...
              <c:otherwise> <!-- 파일이 없거나 이미지가 아닌 경우 출력 -->
							  ...
							  <c:choose>
							    ...
							    <c:when test="${size1 == 0 }"> <!-- 기본 이미지 출력 -->
    							  <a href="./read.do?contentsno=${contentsno}&cateno=${cateno}&word=${param.word}&now_page=${param.now_page}"><IMG src="/contents/images/none2.png" style="width: 120px; height: 80px;"></a>
							    </c:when>
							  </c:choose>
          ...
          <td style='vertical-align: middle;'>
            <a href="./read.do?contentsno=${contentsno}&cateno=${cateno}&word=${param.word}&now_page=${param.now_page}"><strong>${contentsVO.title}</strong><br>
            <c:choose>
              <c:when test="${contentsVO.content.length() > 160 }">
                  ${contentsVO.content.substring(0, 155)}.....
              </c:when>
              <c:when test="${contentsVO.content.length() <= 160 }">
                  ${contentsVO.content}
              </c:when>
          ...
          <td style='vertical-align: middle; text-align: center;'>
            <span style="color: #000000; font-size: 1.2em;">${contentsVO.point} </span><br>
